			...
			<a-form-model-item label="活動名稱" prop="subject">
				...
				<div class="activity-types">
					<template v-for="(item, index) in activityTypeIconList">
						<span
							:class="['type-icon', { active: item.active }]"
							v-if="index === 0"
							:key="index"
							@click="handleActivityType(item, index)"
...
export default {
	...
	computed: {
		...mapGetters(['activityTypeList', 'userInfo']),
	},
	created() {
		...
		validate() {
			return new Promise(resolve => {
				this.form.publicDescription = this.form.publicDescription
					? this.form.publicDescription.trim()
					: this.form.publicDescription
...
.activity-types {
	...
	.type-icon {
		...
		&.active {
			box-shadow: inset 0 0 0 1px #317ae2, 0 0 0 rgba(49, 122, 226, 0), inset 0 1px 2px rgba(49, 122, 226, 0.16);
			background: #eff5fd;
			/deep/ .anticon {
				color: #317ae2;
				fill: #317ae2;
